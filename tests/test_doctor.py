...
def test_all_doctor_warnings_match_frozen_keys_and_codes(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """Every warning generated by Stage.diagnose() must have frozen keys and known codes."""
    stage = Stage(tmp_path / ".stage-signal")
    stage.init(project="warn-freeze")
    stage.start(stage="task", pid=99999999)
    monkeypatch.setattr("stage_signal.stage._is_pid_alive", lambda pid: False)
    diag = stage.diagnose(stale_after=1.0)
    assert diag["warnings"]
    for w in diag["warnings"]:
        assert isinstance(w, dict)
        assert set(w.keys()) >= set(WARNING_KEYS)
