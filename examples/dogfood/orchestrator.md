...
`wait --json --needs-reclaim` uses the same payload shape as `wait --json` (`outcome`, `wanted`, `observed_state` / `state`, `exit_code`, `timeout`, `stage_id`, `dir`, `reason`, `status`) with `wanted: "needs_reclaim"` and an always-present top-level `needs_reclaim` boolean (also nested on `status`). The [watchdog example](../orchestrator-watchdog.sh) exposes this as `--wait-reclaim`.
...
See [`examples/github-action-wait-reclaim.yml`](../github-action-wait-reclaim.yml) and [`examples/github-action-wait.yml`](../github-action-wait.yml).
