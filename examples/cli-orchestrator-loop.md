...
- Stuck `running` (DEAD_PID / STALE) is `wait --needs-reclaim` → `reclaim` → `start`, not a `doctor` sleep loop. See `orchestrator-watchdog.sh --wait-reclaim`.
