...
stage-signal doctor [--stale-after SEC] [--exit-reclaim] [--json] [--format human|json]
...
|------|----------|---------|
| 0 | `EXIT_OK` | Success / wait condition met / `wait --needs-reclaim` met |
| 1 | `EXIT_ERROR` | Error (corrupt status, I/O error, doctor problems, or `wait --needs-reclaim` closed on terminal without reclaim) |
| 2 | `EXIT_BAD_ARGS` | Invalid CLI arguments or malformed inputs |
| 3 | `EXIT_ILLEGAL_TRANSITION` | Illegal lifecycle transition or failed `--require-proof` gate |
| 10 | `EXIT_STATE_RUNNING` | Observed state is `running`; or `doctor --exit-reclaim` when `needs_reclaim` is true |
...
with Stage.open(".stage-signal") as s:
    ...
    s.start(stage="task-001", session_id="ses_abc", pid=12345)
    s.heartbeat(note="processing batch 1")
    s.note("checkpoint reached")
    s.artifact("dist/output.tar.gz", label="build-artifact")
    s.done(summary="task completed successfully", git_head="c4585b6")
    print(s.status())
